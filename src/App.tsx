...
function countNotesInRange(notes: DailyNote[], type: NoteType, dates: DayRecord[]) { const dateSet = new Set(dates.map(day => day.date)); return notes.filter(note => note.type === type && dateSet.has(note.timestamp.slice(0, 10))).reduce<Record<string, number>>((acc, note) => ({ ...acc, [note.label]: (acc[note.label] || 0) + 1 }), {}) }
...
export function App() {
  ...
  function localSummary() {
    ...
    return `近 7 天记录：计划 ${planned} 次，完成 ${done} 次，漏服 ${missed} 次，延后 ${snoozed} 次，完成率约 ${rate}%。\n${refillLine()}\n最近记录的小感受：${formatCounts(countNotesInRange(state.dailyNotes, 'feeling', state.weekRecords))}。\n记录过的漏服原因：${formatCounts(countNotesInRange(state.dailyNotes, 'missedReason', state.weekRecords))}。\n本摘要仅整理您的记录，不构成诊断、处方或治疗建议。`
  }
  function localQuestions() {
    const missed = state.weekRecords.reduce((sum, day) => sum + day.missed, 0)
    return `复诊时可向医生咨询：\n1. ${refillLine()}\n2. 近 7 天有 ${missed} 次漏服记录，其中原因包括：${formatCounts(countNotesInRange(state.dailyNotes, 'missedReason', state.weekRecords))}，是否需要调整提醒方式？\n3. 近期记录的小感受包括：${formatCounts(countNotesInRange(state.dailyNotes, 'feeling', state.weekRecords))}，是否需要向医生说明？\n\n本清单仅整理您的记录，不构成诊断、处方或治疗建议。`
  }
  ...
  return <main className="min-h-screen bg-[#fbf8f3] pb-28 text-[#4a3f35]"><div className="mx-auto max-w-6xl px-4 py-5 sm:px-6 lg:px-8"><AppHeader resetDemo={resetDemo} /><TabNav activeTab={activeTab} onChange={setActiveTab} /><section className="mt-6">{activeTab === 'dashboard' && <DashboardPage {...context} />}{activeTab === 'checkin' && <CheckInPage {...context} />}{activeTab === 'plan' && <MedicationPlanPage {...context} />}{activeTab === 'inventory' && <InventoryPage {...context} />}{activeTab === 'assistant' && <AssistantPage {...context} />}</section><SafetyNotice /></div>{toast && <div className="fixed bottom-24 left-1/2 z-50 max-w-[92vw] -translate-x-1/2 rounded-2xl bg-[#6f7f5f] px-5 py-3 text-center text-base font-semibold text-white shadow-xl shadow-amber-200/40">{toast}</div>}{state.futureModal && <Modal title="说明" onClose={() => setState(current => ({ ...current, futureModal: null }))}>{state.futureModal}</Modal>}</main>
}

function AppHeader({ resetDemo }: { resetDemo: () => void }) { return <header className="rounded-[2rem] bg-[#fffaf2] p-6 shadow-lg shadow-amber-100/50"><div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-base font-semibold text-[#7c6f64]">{friendlyDate}</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-[#4a3f35] sm:text-4xl">今天也慢慢来</h1><p className="mt-3 max-w-3xl text-lg leading-8 text-[#6d6257]">我会帮您记着今天的药、库存和复诊时间。用药决定还是听医生和药师的。</p></div><button onClick={resetDemo} className="min-h-12 rounded-2xl bg-[#e9dcc8] px-5 py-3 text-base font-bold text-[#5b4b3d] shadow-sm hover:bg-[#dfcfb7]">恢复默认数据（将清除记录）</button></div><div className="mt-5 flex flex-wrap gap-2 text-sm font-semibold">{['仅作记录', '不改药', '不替代医生', '演示数据'].map(label => <span key={label} className="rounded-full bg-[#f2eadf] px-3 py-1 text-[#6f5f50]">{label}</span>)}</div></header> }
